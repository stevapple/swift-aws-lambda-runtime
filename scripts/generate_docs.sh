...
modules=(AWSLambdaRuntimeCore AWSLambdaRuntime AWSLambdaEvents)
...
[[ -d swift-aws-lambda-runtime.xcodeproj ]] || swift package generate-xcodeproj
...
jazzy_args=(--clean
            --author 'SwiftAWSLambdaRuntime team'
            --readme "$module_switcher"
            --author_url https://github.com/swift-server/swift-aws-lambda-runtime
            --github_url https://github.com/swift-server/swift-aws-lambda-runtime
            --github-file-prefix https://github.com/swift-server/swift-aws-lambda-runtime/tree/$version
            --theme fullwidth
            --xcodebuild-arguments -scheme,swift-aws-lambda-runtime-Package)
cat > "$module_switcher" <<"EOF"
# SwiftAWSLambdaRuntime Docs

SwiftAWSLambdaRuntime is a Swift implementation for the AWS Lambda Runtime.
...
for module in "${modules[@]}"; do
  ...
  args=("${jazzy_args[@]}" --output "$jazzy_dir/docs/$version/$module" --docset-path "$jazzy_dir/docset/$version/$module"
        ...
        --root-url "https://swift-server.github.io/swift-aws-lambda-runtime/docs/$version/$module/")
  if [[ -f "$root_path/.build/sourcekitten/$module.json" ]]; then
...
if [[ $PUSH == true ]]; then
  ...
  git add --all docs
  echo '<html><head><meta http-equiv="refresh" content="0; url=docs/current/AWSLambdaRuntimeCore/index.html" /></head></html>' > index.html
  git add index.html
  touch .nojekyll
  git add .nojekyll
